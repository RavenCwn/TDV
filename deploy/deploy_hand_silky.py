...
[-0.09659995, 0.03491090, 0.99471090, 0.06544165],
[-0.88963582, -0.45118648, -0.07056064, -0.00982884],
[0.44633674, -0.89174662, 0.07464256, 0.02979702],
[0.00000000, 0.00000000, 0.00000000, 1.00000000]
    ...
    # for i in tqdm(range(5)):
        ...
        # for smooth_action in arm:
            ...
            if DEBUG:
                ...
                """
                # 执行bg
                generated_trajectory.append(T_b_g) 

                # 在原有T_b_g位置基础上，直接计算新的相对位置
                relevant_pose_T = np.linalg.inv(T_b_cam_init @ T_cam_ref_init) @ T_b_g @ np.linalg.inv(T_hand_g_init)
                relative_pose = T_to_6D(relevant_pose_T, rot_type="6d")
    ...
    # 直接执行所有轨迹中的历史数据
    generated_trajectory_6d = [T_to_6D(pose, '6d') for pose in generated_trajectory]
    rob.movexs('movel', generated_trajectory_6d)
