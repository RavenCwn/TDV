...
import math
from T import T_b_cam_init as T_b_cam_init
lc = lcm.LCM()

DEBUG = 1 # 0：机器人不动；1：机器人动；2: 复位；12
...
def normalize_traj(traj: np.ndarray,
                   pos_mean: np.ndarray,
                   pos_var: np.ndarray,
                   eps: float = 1e-8) -> np.ndarray:
    """
    对轨迹进行正向归一化（standardization），只处理前三维 (x, y, z)。

    参数
    ----
    traj : np.ndarray, shape (T, D)
        原始轨迹数据，第一维是时间步，第二维至少包含前三个位置通道。
    pos_mean : np.ndarray, shape (3,)
        用于标准化的均值向量。
    pos_var : np.ndarray, shape (3,)
        用于标准化的方差向量。
    eps : float
        数值稳定项，加在 sqrt(var) 上，应与反归一化时一致。

    返回
    ----
    norm_traj : np.ndarray, shape (T, D)
        标准化后的轨迹数据，前三维为 (x-mean)/sqrt(var+eps)，其余维度不变。
    """
    # 复制输入，避免原地修改
    norm_traj = traj.copy()
    
    # 计算标准差 scale
    scale = np.sqrt(pos_var) + eps  # shape (3,)
    
    # 只对前三维做标准化
    norm_traj[:, :3] = (traj[:, :3] - pos_mean) / scale
    
    return norm_traj


def denormalize_traj(normalized_traj: np.ndarray,
                     pos_mean: np.ndarray,
                     pos_var: np.ndarray,
                     eps: float = 1e-8) -> np.ndarray:
    """
    ...
    参数
    ----
    normalized_traj : np.ndarray, shape (T, D)
        归一化后的轨迹数据，前 3 维是 (x,y,z)。
    pos_mean : np.ndarray, shape (3,)
        标准化时使用的均值。
    pos_var : np.ndarray, shape (3,)
        标准化时使用的方差。
    eps : float
        为了数值稳定性，加在 sqrt(var) 上的小量，需与标准化时保持一致。
    
    返回
    ----
    real_traj : np.ndarray, shape (T, D)
        恢复到原始坐标系下的轨迹，前三维还原后，其他维保持输入不变。
    """
    # 先拷贝，避免修改原数组
    real_traj = normalized_traj.copy()
    
    # 计算 scale = sqrt(var) + eps
    scale = np.sqrt(pos_var) + eps  # shape (3,)
    
    # 对前 3 维做反归一化： x = x_norm * scale + mean
    real_traj[:, :3] = normalized_traj[:, :3] * scale + pos_mean
...
#  ])
    ...
    # 获取当前任务的标准差和方差,提取归一化数据
    pos_mean = np.load(f'{args.ckpt_dir}/pos_mean.npy')
    pos_var = np.load(f'{args.ckpt_dir}/pos_var.npy')
...
#     )
    ...
    relative_pose = normalize_traj(relative_pose, pos_mean=pos_mean, pos_var=pos_var)
    # time.sleep(1)
    ...
    # for i in tqdm(range(5)):
        ...
        # for smooth_action in arm:
            ...
            反向normalize,提取执行路径
            '''
            smooth_action = denormalize_traj(smooth_action, pos_mean=pos_mean, pos_var=pos_var)
...
def parse_args():
    ...
    parser.add_argument('--ckpt_dir', '-c', default='results/coordiff_real/coordiff_mlp_rms_leaf/teapot_coaster_single_01234_360_d_norl',type=str, help='checkpoint dir.')
