...
class FrameSaver:
    def __init__(self, folder):
        ...
        # self.config.enable_device('250122073881')
        # self.config.enable_device('408322070054')
    ...
    def save_frame(self):
        ...
        """
        frames = self.pipeline.wait_for_frames()    # 让RealSense阻塞式等待，直到采到最新的帧数据
        aligned_frames = self.align.process(frames)     # 对帧进行对齐

        aligned_depth_frame = aligned_frames.get_depth_frame()  # 提取对齐后的depth
        color_frame = aligned_frames.get_color_frame()  # 提取对齐后的rgb
        ...
        cv2.imwrite(filedepth, depth_data)
