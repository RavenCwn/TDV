...
@timed("GDSAM2切割")
def gdsam2_process_frame(processor, image_path, mask_path, input_image_path, output_mask_path, mask_vis_path):
    return processor.process_frames(image_path, mask_path, input_image_path, output_mask_path, mask_vis_path)
...
def capture_and_process(is_only_first, sam2_checkpoint, model_cfg, obj_cad_path, ref_cad_path, sam_tmp_dir, data_dir, track_vis_dir, mask_vis_dir, track_pose_path, segmentor_model, stability_score_thresh, use_parallel_mode, rounds, obj_text_prompt, ref_text_prompt):
    # 制定相机临时存放地
    camera_dir = f'captured/{obj_name}/'
    # 删除旧目录并重新创建
    ...
    # *立刻* 定义所有路径
    rgb_path      = os.path.join(camera_dir, "rgb.png")
    depth_path    = os.path.join(camera_dir, "depth.png")
    obj_mask_path = os.path.join(camera_dir, "obj_mask.png")
    ref_mask_path = os.path.join(camera_dir, "ref_mask.png")
    image_path    = os.path.join(camera_dir, "image.png")
    
    fp_processor_obj = create_fp_processor(model_path=obj_cad_path, camera_info=cam_info)
    fp_processor_ref = create_fp_processor(model_path=ref_cad_path, camera_info=cam_info)

    # # 初始化GDSAM2处理示例
    gdsam2_processor = create_gdsam2_processor(model_cfg=model_cfg, sam2_checkpoint=sam2_checkpoint)

    def do_initial_detection():
        # object
        gdsam2_processor.predict_mask(
            rgb_path=rgb_path,
            mask_path=obj_mask_path,
            text_prompt=obj_text_prompt
        )
        # reference
        gdsam2_processor.predict_mask(
            rgb_path=rgb_path,
            mask_path=ref_mask_path,
            text_prompt=ref_text_prompt
        )
        # preserve this RGB as the “template” image
        shutil.copy(rgb_path, image_path)
    
        print("Re-ran initial-frame segmentation and re-initialized FP processors.")
    
    # 初始化计数器
    frame_count = 0  
    # do_initial_detection()

    # 确定obj_text_prompt小写且以.结尾
    obj_text_prompt = obj_text_prompt.lower().strip()
    if not obj_text_prompt.endswith("."):
        obj_text_prompt += "."
    ref_text_prompt = ref_text_prompt.lower().strip()
    if not ref_text_prompt.endswith("."):
        ref_text_prompt += "."
    ...
    while True:
        ...
        obj_mask_path = os.path.join(camera_dir, f"obj_mask.png")
        ref_mask_path = os.path.join(camera_dir, f"ref_mask.png")
        image_path = os.path.join(camera_dir, f"image.png")
        ...
        if frame_count == 0:    # 仅对首帧进行处理
            # obj 
            gdsam2_processor.predict_mask(rgb_path=rgb_path, mask_path=obj_mask_path, text_prompt=obj_text_prompt)
            obj_mask = cv2.imread(obj_mask_path, cv2.IMREAD_GRAYSCALE)

            # refm
            gdsam2_processor.predict_mask(rgb_path=rgb_path, mask_path=ref_mask_path, text_prompt=ref_text_prompt)
            ref_mask = cv2.imread(ref_mask_path, cv2.IMREAD_GRAYSCALE)

            shutil.copy(rgb_path, image_path)     # RGB保留下来
            ...
            # obj
            gdsam2_process_frame(processor=gdsam2_processor, obj_mask_path=obj_mask_path, image_path=image_path, input_image_path=rgb_path, output_mask_path=obj_mask_path, mask_vis_path=mask_vis_path)     
            obj_mask = cv2.imread(obj_mask_path, cv2.IMREAD_GRAYSCALE)  

            # ref 
            gdsam2_process_frame(processor=gdsam2_processor, obj_mask_path=ref_mask_path, image_path=image_path, input_image_path=rgb_path, output_mask_path=ref_mask_path, mask_vis_path=mask_vis_path)     
            ref_mask = cv2.imread(ref_mask_path, cv2.IMREAD_GRAYSCALE)  

            shutil.copy(rgb_path, image_path) # RGB保留下来
        
        # gdsam2_process_frame(processor=gdsam2_processor, ref_mask_path=mask_path, image_path=rgb_path, input_image_path=rgb_path, output_mask_path=mask_path, mask_vis_path=mask_vis_path)
        
        # ************用GD SAM2进行checking************ #
        # gdsam2_process_frame(processor=gdsam2_processor, ref_mask_path=ref_mask_path, image_path=image_path, intput_image_path=rgb_path, output_mask_path="output/ljc.png")
        # gdsam2_processor.process_frames(ref_mask_path=ref_mask_path, image_path=image_path, input_image_path=rgb_path, output_mask_path=mask_path)
        ...
        obj_pose, time = fp_process_frame(
            processor=fp_processor_obj,
            color_img=color_img,
            depth_map=depth_map,
            mask=obj_mask,
            obj_id=1,
            frame_id=frame_count,
            camera_matrix=fp_processor_obj.config['camera_matrix'],
            rounds=rounds
        )

        ref_pose, time = fp_process_frame(
            processor=fp_processor_ref,
            color_img=color_img,
            depth_map=depth_map,
            mask=ref_mask,
            obj_id=1,
            frame_id=frame_count,
            camera_matrix=fp_processor_ref.config['camera_matrix'],
            rounds=rounds
        )

        # relevant_pose = np.linalg.inv(T_cam_ref_init) @ obj_pose
        relevant_pose = np.linalg.inv(ref_pose) @ obj_pose

        
        relevant_msg = Matrix4x4Flat()
        relevant_msg.matrix = relevant_pose.flatten().tolist()  # 将矩阵数据转化为列表
        lc.publish("relevant_pose", relevant_msg.encode())
        print("Broadcasting relevant_msg")

        # if frame_count == 10:
        np.save("/home/user/pgp/CoorDiff/pose/obj_pose.npy", obj_pose)
        np.save("/home/user/pgp/CoorDiff/pose/ref_pose.npy", ref_pose)


        # 可视化
        obj_vis_img = fp_processor_obj.visualize_result(color_img, obj_pose, fp_processor_obj.config['camera_matrix'])
        ref_vis_img = fp_processor_ref.visualize_result(color_img, ref_pose, fp_processor_ref.config['camera_matrix'])
        cv2.imshow("Ref tracking", ref_vis_img[..., ::-1]) 
        cv2.imshow("Obj tracking", obj_vis_img[..., ::-1]) 
...
if __name__ == "__main__":
    ...
    time = 6
    obj_name = "bluemug"
    obj_prompt = 'blue mug with yellow lining'
    ref_name = "pinkmug"
    ref_prompt = 'pink mug behind another mug'

    # obj_name = "teapot2"
    # obj_prompt = 'black teapot'
    # ref_name = "coastercat"
    # ref_prompt = 'yellow coaster with cat'

    # obj_name = "flower3"
    # obj_prompt = 'flower'
    # ref_name = "duck"
    # ref_prompt = 'vase with a shape like duck'

    # obj_name = "pinkmug"
    # obj_prompt = 'pink mug'
    # ref_name = "mug_holder2"
    # ref_prompt = 'wooden cup holder'

    # obj_name = "steve"
    # obj_prompt = 'blue doll'
    # ref_name = "card"
    # ref_prompt = 'purple and white card with complex pattern'

    # obj_name = "xiayoujie"
    # obj_prompt = 'yellow base figurine with black hair'
    # # obj_name = "chanzi"
    # # obj_prompt = 'brown handle with black shovel'
    # ref_name = "chanzi"
    # ref_prompt = 'wooden handle shovel'

    
    # ref_name = "drawer_lllast"
    # ref_prompt = 'green drawer'
    # obj_name = ref_name
    # obj_prompt = ref_prompt
    ...
    # input mesh
    parser.add_argument("--obj_cad_path", default=f"Data/real_data/{obj_name}{id}_mesh/{obj_name}{id}.obj", help="Path to CAD model")  # 注意输入单位为mm，中间处理为m
    parser.add_argument("--ref_cad_path", default=f"Data/real_data/{ref_name}{id}_mesh/{ref_name}{id}.obj", help="Path to CAD model")  # 注意输入单位为mm，中间处理为m
    ...
    parser.add_argument("--sam_tmp_dir", default=f"Data/real_data/{obj_name}{id}_mesh/{obj_name}{id}_tmp", help="tmp directory")
    ...
    parser.add_argument("--track_vis_dir", default=f"output/{obj_name}{id}/{obj_name}{id}_track_vis", help="rgb, depth and mask")
    parser.add_argument("--mask_vis_dir", default=f"output/{obj_name}{id}/{obj_name}{id}_mask_vis", help="rgb, depth and mask")
    parser.add_argument("--track_pose_path", default=f"output/{obj_name}{id}", help="rgb, depth and mask")
    ...
    pose = capture_and_process(
        ...
        obj_text_prompt=obj_prompt,
        ref_text_prompt=ref_prompt,

        # obj_text_prompt='teapot',
        # ref_text_prompt='coaster with cat',
...
# teapot
# [[ 0.08089248, -0.72580944,  0.68312328, -0.04591928],
#  [ 0.67367689, -0.46530341, -0.57415259, -0.09529585],
#  [ 0.7345854 ,  0.50664913,  0.45132179,  0.68078846],
#  [ 0.        ,  0.        ,  0.        ,  1.        ]]

# coaster
# [[-0.10608897, -0.99418712,  0.01828166,  0.19553925],
#  [-0.64063525,  0.08239949,  0.76341078, -0.10655618],
#  [-0.76048059,  0.06927749, -0.64565396,  0.68848108],
#  [ 0.        ,  0.        ,  0.        ,  1.        ]]