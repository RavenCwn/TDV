...
def save_data(state, output_folder):
    ...
    with open(os.path.join(output_folder, 'task_related_obj.txt'), 'w') as file: 
        if state == 0:
            file.write('gripper_pose jar_lid0')
        elif state == 1:
            file.write('jar_lid0 jar0')
...
origin_path = "mydata3/close_jar/variation0"

output_path = "mydata3_re/close_jar"

for i, f in enumerate(os.listdir(origin_path + '/episodes')):
    print(i)
    state = 0
    grasp = False
