...
def main(cfg: DictConfig):
    ...
    gripper_train_dataset = TrajDataset(
        dataset_dir=cfg.train_dataset,
        mode='train',
        task_list=cfg.task_list,
        **cfg.gripper_dataset_cfg
    )
    gripper_val_dataset = TrajDataset(
        dataset_dir=cfg.val_dataset,
        mode='val',
        task_list=cfg.task_list,
        **cfg.gripper_dataset_cfg)
    ...
    gripper_train_loader = get_dataloader(gripper_train_dataset,
                                mode="train",
                                num_workers=cfg.num_workers,
                                batch_size=cfg.batch_size)
    
    gripper_val_loader = get_dataloader(gripper_val_dataset,
                                mode="val",
                                num_workers=cfg.num_workers,
                                batch_size=cfg.batch_size)
    ...
    for epoch in metric_logger.log_every(range(cfg.epochs), 1, ""):
        train_metrics = run_one_epoch(
            ...
            gripper_dataloader = gripper_train_loader,
        ...
        if epoch % cfg.val_freq == 0:
            val_metrics = evaluate(
                ...
                gripper_dataloader = gripper_val_loader,
                device=device,
                mix_precision=cfg.mix_precision,
                tag="val",
...
def run_one_epoch(model,
    ...
    weights = torch.tensor([0.01, 1]).to(device)  # 示例权重
    ...
    for hist, actions_gt, task_emb, state, gripper_change in tqdm(gripper_dataloader):
        hist = hist.to(device)
        actions_gt = actions_gt.to(device)
        task_emb = task_emb.to(device)
        state = state.to(device)
...
@torch.no_grad()
def evaluate(model, DDIM: DDIMScheduler, dataloader, gripper_dataloader, device, mix_precision=False, tag="val", eval_timesteps=8):
    tot_loss_dict, tot_items = {}, 0
    arm_model, gripper_model = model
    arm_model.eval()
    gripper_model.eval()
