...
cd /home/user/ljc/AutoPoseEstimator
conda activate auto_pose
# python gen_pose_move_scale_auto_dilute.py --need_rot --num_samples 100 --task ${task_name} --obj_name ${obj_name} --ref_name ${ref_name} 
python gen_pose_move_scale_auto_dilute.py --num_samples 100 --task ${task_name} --obj_name ${obj_name} --ref_name ${ref_name} 
task_name="${task_name}_gen_test3" 
...
# # 第五步：开始训练
cd ..
python run_multi_real.py -d coordiff_real_world/recollection_final -c coordiff_mlp_rms -w coordiff_mlp_rms_leaf -t1 ${task_name}
